...
folder_name_list = [""]  #'training_images'
...
class SimpleSelectBoxDeleteLabel:
    def __init__(self):
        ...
        labelShow = tk.Label(
            self.window, text="Please enter # (number of box to delete)"
        )
        ...
        label.bind(
            "<Button-1>", lambda event, text="OK": self.hLabelClickOK(event, text)
        )
    ...
    def __init__(self, labelList, nCol=6):
        ...
        while 1:
            ...
            for j in range(0, numInRow):
                ...
                label.bind(
                    "<Button-1>",
                    lambda event, text=txtLabel: self.hLabelClick(event, text),
                )
...
while True:
    if img_index_changed:
        ...
        for idx, marked_cvRect in enumerate(marked_cvRects):
            ...
            cv.rectangle(
                show_original_image, (xc, yc), (xc + wc, yc + hc), labelColor, 4
            )
            cv.putText(
                show_original_image,
                "#" + str(idx) + " " + marked_Label[idx],
                (xc, yc),
        ...
        if mouseDragging:
            ...
            cropRect = cvRect(
                [
                    mouseFirstPoint[0],
                    mouseFirstPoint[1],
                    abs(mouseLastPoint[0] - mouseFirstPoint[0]),
                    abs(mouseLastPoint[1] - mouseFirstPoint[1]),
                ]
            )
            (hImg, wImg) = show_original_image.shape[:2]
            if cropRect.x < 0:
            ...
            if (
                (cropRect.area() >= 500)
                and (cropRect.w + cropRect.x <= wImg - 1)
                and (cropRect.h + cropRect.y <= hImg - 1)
            ):  ## large_enough & not bigger than the original_image
                cropRectOK = True
                (hImg, wImg) = show_original_image.shape[:2]
                cv.rectangle(
                    show_original_image,
    ...
    if key == ord("q") or key == ord("Q"):  # 'q' -> exit
        ...
        key == 2424832 or key == 2490368 or key == ord("a")
    ):  # ←/↑ or a goto previous image
        print("--")
        img_index -= 1
        img_index_changed = True
        cropRectOK = False
        ...
        key == 2555904 or key == 2621440 or key == ord("d")
    ):  # →/↓ or d goto next image
        print("++")
        img_index += 1
        img_index_changed = True
        cropRectOK = False
