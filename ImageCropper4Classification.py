...
dataset_path = "./DrowningDetectionDataset/Broken/"
...
def ProcessInEachFolder():
    ...
    for i, fname in enumerate(list_files):
        ...
        for idx, xyhw in enumerate(xywhs):
            ...
            # crop
            save_crop_image_path = (
                dataset_output_crop_path
                + "/"
                + label
                + "/"
                + label
                + "_"
                + str(countingList[id_label]).zfill(5)
                + "."
                + save_img_extension
            )
            newPadded = cvRect(
                [
                    xyhw.x - paddingHorizontal,
                    xyhw.y - paddingVertical,
                    xyhw.w + (paddingHorizontal * 2),
                    xyhw.h + (paddingVertical * 2),
                ]
            )
            # check valid
            padding_valid = True
            ...
            cropped_image = None
            if padding_valid:
                cropped_image = cropping_img[
                    newPadded.y : newPadded.y + newPadded.h,
                    newPadded.x : newPadded.x + newPadded.w,
                ]
            else:
                cropped_image = cropping_img[
                    xyhw.y : xyhw.y + xyhw.h, xyhw.x : xyhw.x + xyhw.w
                ]
            ...
            if (cropped_image != None).any():
                ...
                print(
                    f"Error: cropping in {single_image_path} [{xyhw.x},{xyhw.y},{xyhw.w},{xyhw.h}] skipped {save_crop_image_path}"
                )
